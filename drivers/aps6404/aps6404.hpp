...
namespace pimoroni {
    class APS6404 {
        public:
            ...
            void write(uint32_t addr, const uint32_t* data, uint32_t len_in_bytes);
            void write_repeat(uint32_t addr, uint32_t data, uint32_t len_in_bytes);

            // Write and block until completion
            void write_blocking(uint32_t addr, const uint32_t* data, uint32_t len_in_bytes) {
                write(addr, data, len_in_bytes);
        ...
        private:
            void write_no_page_crossing(uint32_t addr, const uint32_t* data, uint32_t len_in_bytes);
